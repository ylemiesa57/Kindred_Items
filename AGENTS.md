...
- `GROQ_API_KEY` (in `.env`, copied from `.env.example`) is only used server-side for `POST /api/transcribe` (Whisper) and `POST /api/world/analyze` (vision / World Mode). Without a valid key those two endpoints return `503`. The core product — introducing/enrolling object twins, conversing with them, and recording stateful memory changes — works fully without Groq using the typed/keyboard inputs (enrollment has a "Need to type instead?" fallback; conversation and "Notice change" use text inputs). `GET /api/health` reports `groqConfigured: true` whenever `.env` has any non-empty value, so it does not validate the key.
